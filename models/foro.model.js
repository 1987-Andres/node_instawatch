const getAllPosts = () => {
    return new Promise((resolve, reject) => {
        db.query('SELECT * FROM heroku_bf6019206e7eabe.nuevo_post', (err, rows) => {
            if (err) reject(err);
            resolve(rows);
        })
    })
...
const getByCategoria = (pCategoria) => {
    return new Promise((resolve, reject) => {
        db.query(`SELECT * FROM heroku_bf6019206e7eabe.nuevo_post where categoria="${pCategoria}"`, (err, rows) => {
            if (err) reject(err);
            resolve(rows);
        })
    })
